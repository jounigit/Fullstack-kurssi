...
class App extends React.Component {
  ...
  render() {
    ...
    const bigggest = Math.max(...this.state.points)
    const getBiggest = this.state.points.indexOf(bigggest)
    console.log("Sattuma: ", lotto)
    console.dir(this.state.points)
    console.log("Most:", getBiggest);
    ...
    return (
      <div>
        ...
        <p>{this.props.anecdotes[getBiggest]}<br />
        has {Math.max(...this.state.points)} votes</p>
      </div>
    )
  }
}
